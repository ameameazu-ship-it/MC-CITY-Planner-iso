...
// 長押し進捗円（iso-engine.jsのlpCircle/stampCircle変数を使用）
// iso-engine.jsで _drawProgressCircle と _startRafLoop が定義されている前提
var lpCircle=null;
...
function recomputeGroups(c,r){
  ...
  var tries=[[c,r,2,2],[c-1,r,2,2],[c,r-1,2,2],[c-1,r-1,2,2]];
...
function placeCell(c,r){
  ...
  if(gW>1||gH>1){
    ...
    triggerBlockAnim(c,r); scheduleRender(); return true;
  }
  var k=ck(c,r);
...
function _doDragMove(toC,toR){
  ...
  var movedData=moveCells.map(function(p){
    var k2=ck(p.c,p.r), cellData=cells[k2]; delete cells[k2];
...
function cancelLongPress(){
  ...
  // 円はここでは消さない（移動閾値超え時のみ消す）
}

function _cancelCirclesOnMove(){
  // 指/マウスが閾値以上動いたときだけ円をキャンセル
  if(longPressTimer){clearTimeout(longPressTimer);longPressTimer=null;}
  lpCircle=null;
}
