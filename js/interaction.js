...
var lpCircle=null;    // {c,r,startTime,duration,color,rad} 長押し移動 進捗円
var stampCircle=null; // {c,r,startTime,duration,color,rad} スタンプ進捗円
...
function placeCell(c,r){
  ...
  if(gW>1||gH>1){
    for(var dc=0;dc<gW;dc++) for(var dr=0;dr<gH;dr++){
      ...
      if(cells[ck(c+dc,r+dr)]) return false;
    }
...
function _eraseAnchor(ac,ar){
  ...
  var cell=cells[ak]; if(cell.ghost) return;
...
function floodFill(c,r){
  ...
  while(queue.length){
    ...
    if(existCell&&existCell.ghost) continue;
...
function onTouchMove(e){
  ...
  if(ts.length>=2&&isPinching){
    ...
    panX=midX-(pinchMid0X-pinchPanX0)*zr;panY=midY-(pinchMid0Y-pinchPanY0)*zr;zoom=nz;
  ...
  _didPlace=false;
  if(tool==='draw'&&stampMode&&_isRoadSelected()){
    // 道路スタンプ：8方向スナップ＋Bresenham補間で直線保証
    stampedSet=new Set();
    var snapped=_snapRoadCell(cell.c,cell.r);
    var fromC=(lastC>=0)?lastC:stampStartC, fromR=(lastR>=0)?lastR:stampStartR;
...
function onTouchEnd(e){
  ...
  if(dragMoveKey){
    if(dragMoveMode){
      ...
      var parts=dragMoveKey.split(',');openCtxMenu(parseInt(parts[0]),parseInt(parts[1]),touchStartX,touchStartY);
    }
...
function onWindowMouseMove(e){
  if(isPanning){
    ...
    panX+=dx*0.55;panY+=dy*0.55;_lastMouseX=e.clientX;_lastMouseY=e.clientY;render();return;
  }
...
function onWindowMouseUp(e){
  ...
  cancelLongPress();stampMode=false;
...
function cancelLongPress(){if(longPressTimer){clearTimeout(longPressTimer);longPressTimer=null;}lpCircle=null;scheduleRender();}
