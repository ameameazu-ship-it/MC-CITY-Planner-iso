...
var k=ck(c,r);
var rid=resolveId(selectedId); // ← バリアントをランダム選択
if(cells[k] && cells[k].id===rid) return;
cells[k]={ id:rid, dir:‘none’ }; // ← resolveId を使用
...
if(!inGrid(c,r)) return;
var targetId = (cells[ck(c,r)]||{}).id||’**empty**’;
...
var curId=(cells[kk]||{}).id||’**empty**’;
if(curId!==targetId) continue;
cells[kk]={id:resolveId(selectedId),dir:‘none’}; // ← 塗り潰しもランダム
