...
function _popScale(t){
  ...
  if(t<0.5){ var u=t/0.5; return 1.28*u*u; }
  else{ var u=(t-0.5)/0.5; return 1.28-0.28*(1-Math.pow(1-u,3)); }
}

function getBlockScale(c, r){
  var key=ck(c,r); if(!blockAnims[key]) return 1;
...
// 長押し成功時のゴールドフラッシュ（バイブ代替ビジュアル）
var dragFlashCells   = [];   // [{c,r}]  フラッシュ対象セルリスト
var dragFlashStart   = 0;    // performance.now() の開始時刻
var FLASH_DURATION   = 600;  // ms：フラッシュ全体の長さ
// パルス回数：フラッシュ中に3回点滅させる（ドン・ふわ・キランに対応）
var FLASH_PULSES     = 3;

function triggerDragFlash(c, r){
  var cellData = getCell(c,r); if(!cellData) return;
  // グループ全体 or 単セル
  var targets=[];
  if(cellData.gid && typeof groupMap!=='undefined' && groupMap[cellData.gid]){
    groupMap[cellData.gid].cells.forEach(function(pos){ targets.push({c:pos.c,r:pos.r}); });
  } else {
    targets.push({c:c,r:r});
  }
  dragFlashCells = targets;
  dragFlashStart = performance.now();
  _startRafLoop();
}

// フラッシュの輝度（0→1→0を3回繰り返す鋸波）
function _getFlashAlpha(){
  ...
  // FLASH_PULSES 回の sin波（0→1→0 を繰り返す）
  var wave = Math.sin(t * Math.PI * FLASH_PULSES);
  return Math.max(0, wave) * 0.85;
}

// ── RAF ループ管理 ───────────────────────────────────────────────
// dirty フラグとは独立して、アニメーション中は毎フレーム描画する
...
function cellToScreen(c,r){
  return { x:panX+(c-r)*HW*zoom, y:panY+(c+r)*HH*zoom };
}
function screenToCell(sx,sy){
  var dx=(sx-panX)/(HW*zoom), dy=(sy-panY)/(HH*zoom);
  return { c:Math.floor((dx+dy)/2), r:Math.floor((dy-dx)/2) };
}
function centerView(){
  var mid=cellToScreen(COLS/2,ROWS/2);
  panX+=cw/2-mid.x; panY+=ch/2-mid.y;
}
...
function drawDiamond(ctx,x,y,fill,stroke,strokeW){
  ...
  ctx.beginPath();
  ctx.moveTo(x,y); ctx.lineTo(x+hw,y+hh); ctx.lineTo(x,y+hh*2); ctx.lineTo(x-hw,y+hh);
  ctx.closePath();
...
function isoBox(ctx,x,y,bh,tc,lc,rc){
  ...
  ctx.beginPath(); ctx.moveTo(x,y-zb); ctx.lineTo(x-hw,y+hh-zb); ctx.lineTo(x-hw,y+hh*2); ctx.lineTo(x,y+hh*2); ctx.closePath(); ctx.fillStyle=lc; ctx.fill();
  ctx.beginPath(); ctx.moveTo(x,y-zb); ctx.lineTo(x+hw,y+hh-zb); ctx.lineTo(x+hw,y+hh*2); ctx.lineTo(x,y+hh*2); ctx.closePath(); ctx.fillStyle=rc; ctx.fill();
  ctx.beginPath(); ctx.moveTo(x,y-zb); ctx.lineTo(x+hw,y+hh-zb); ctx.lineTo(x,y+hh*2-zb); ctx.lineTo(x-hw,y+hh-zb); ctx.closePath(); ctx.fillStyle=tc; ctx.fill();
}

// ── Draw: gable roof EW ──────────────────────────────────────────
function isoGableEW(ctx,x,y,bh,rh,tc,lc,rc){
  var hw=HW*zoom,hh=HH*zoom,zb=bh*zoom,zr=rh*zoom,by=y-zb;
...
function isoHipRoof(ctx,x,y,bh,rh,lc,rc){
  var hw=HW*zoom,hh=HH*zoom,zb=bh*zoom,zr=rh*zoom,by=y-zb;
  var N={x:x,y:by},E={x:x+hw,y:by+hh},S={x:x,y:by+2*hh},W={x:x-hw,y:by+hh},tip={x:x,y:by-zr};
...
function isoCylinder(ctx,x,y,bh,r,tc,bc){
  var zb=bh*zoom,zr=r*zoom,hh=HH*zoom,cx2=x,cy2=y+hh;
  ctx.save();
  var grad=ctx.createLinearGradient(cx2-zr,cy2,cx2+zr,cy2);
  grad.addColorStop(0,shadeC(bc,0.55)); grad.addColorStop(0.4,bc); grad.addColorStop(1,shadeC(bc,0.72));
...
function winL(ctx,x,y,zbh,u,v,wu,vh,c){
  var hw=HW*zoom,hh=HH*zoom,ax=x-hw,ay=y+hh;
  ctx.beginPath();
  ctx.moveTo(ax+u*hw,ay-u*hh-v*zbh); ctx.lineTo(ax+(u+wu)*hw,ay-(u+wu)*hh-v*zbh);
  ctx.lineTo(ax+(u+wu)*hw,ay-(u+wu)*hh-(v+vh)*zbh); ctx.lineTo(ax+u*hw,ay-u*hh-(v+vh)*zbh);
  ctx.closePath(); ctx.fillStyle=c; ctx.fill();
}
function winR(ctx,x,y,zbh,u,v,wu,vh,c){
  var hw=HW*zoom,hh=HH*zoom;
  ctx.beginPath();
  ctx.moveTo(x+u*hw,y+u*hh-v*zbh); ctx.lineTo(x+(u+wu)*hw,y+(u+wu)*hh-v*zbh);
  ctx.lineTo(x+(u+wu)*hw,y+(u+wu)*hh-(v+vh)*zbh); ctx.lineTo(x+u*hw,y+u*hh-(v+vh)*zbh);
  ctx.closePath(); ctx.fillStyle=c; ctx.fill();
}
...
function _renderNow(){
  ...
  if(nightMode){
    ...
    sky.addColorStop(0,pal[5]||shadeC(pal[0],0.28)); sky.addColorStop(1,pal[6]||shadeC(pal[1],0.22));
  ...
  var tileList=[];
  for(var r2=0;r2<ROWS;r2++) for(var c2=0;c2<COLS;c2++) tileList.push([c2,r2]);
  ...
  if(typeof groupMap!=='undefined'){
    ...
    tileList.forEach(function(cr){
      ...
      var fs=Math.max(9,Math.min(15,zoom*11)),lbl=bbox.w+'×'+bbox.h;
      gctx.save(); gctx.font='bold '+fs+'px sans-serif'; gctx.textAlign='center'; gctx.textBaseline='bottom';
      var tw=gctx.measureText(lbl).width;
      gctx.fillStyle='rgba(0,0,0,0.70)'; gctx.fillRect(labelX-tw/2-3,labelY-fs-2,tw+6,fs+4);
      gctx.fillStyle='#f5c842'; gctx.fillText(lbl,labelX,labelY); gctx.restore();
    });
  }

  // ── Pass 4: ドラッグフラッシュ（3回点滅）────────────────────
  var flashAlpha = _getFlashAlpha();
  if(flashAlpha > 0 && dragFlashCells.length > 0){
    dragFlashCells.forEach(function(pos){
      var s=cellToScreen(pos.c,pos.r);
      // 内側を明るいゴールドで塗りつぶし
      drawDiamond(gctx,s.x,s.y,'rgba(255,220,50,'+flashAlpha+')',null);
      // 縁取りは白でくっきり
      drawDiamond(gctx,s.x,s.y,null,'rgba(255,255,255,'+(flashAlpha*0.9)+')',2.5);
    });
  }

  // ── Pass 5: ドラッグ中ハイライト（常時点灯）────────────────
  if(typeof dragHighlightKey!=='undefined'&&dragHighlightKey){
    ...
    dhKeys.forEach(function(pos){
      var s=cellToScreen(pos.c,pos.r);
      drawDiamond(gctx,s.x,s.y,'rgba(255,200,50,0.25)','rgba(255,200,50,0.90)',2.2);
    });
  }
...
function drawGeneric(ctx,x,y,id,dir,b){
  ...
  var hh=HH*zoom,fs=Math.max(8,Math.min(22,zoom*18));
  ctx.save(); ctx.font=fs+'px serif'; ctx.textAlign='center'; ctx.textBaseline='middle';
  ctx.fillText(b.icon||'?',x,y-bh*zoom+hh*0.5); ctx.restore();
}
